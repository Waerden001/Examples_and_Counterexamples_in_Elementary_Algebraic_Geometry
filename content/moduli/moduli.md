...
Check out [On the Gieseker Harder-Narasimhan filtration for principal bundles](https://arxiv.org/pdf/1411.2884.pdf)

### Example: $\mathbb{P}^{1}\times \mathbb{P}^{1}$ and change of polarization {#ecag-0309}

See here [Notions of stability of sheaves](http://www.northeastern.edu/iloseu/Xiaolei_S16.pdf)

### Example: A rank $2$ moduli space, $\mathrm{K}3$ surface of degree $8$ in $\mathbb{P}^{5}$  {#ecag-0310}

[page 7](http://www.northeastern.edu/iloseu/Barbara_S16.pdf)
...
[Jordan-Holder filtration v.s. Harder-Narasimhan filtration](https://mathoverflow.net/questions/180551/jordan-holder-vs-harder-narasimhan)

### Example: Strong semistability is not an open property {#ecag-0313}

[Strong semistability is not an open property](http://www.ams.org/books/pspum/080.1/2483939/pspum080.1-2483939.pdf)
...
[Here](https://mathoverflow.net/questions/3742/examples-of-algebraic-stacks-without-coarse-moduli-space?rq=1).

### Example: $x^{2}+y^{3}=z^{7}$ {#ecag-0326}

[Twists of $X(7)$ and primitive solutions to $x^{2](http://math.mit.edu/~poonen/papers/pss.pdf)+y^3=z^7$}.

### Example: $(p-1)/24$ supersingular elliptic curves in characteristic $p$ {#ecag-0327}

[Here](https://mathoverflow.net/questions/24573/is-there-a-nice-proof-of-the-fact-that-there-are-p-1-24-supersingular-elliptic)

### Example: Mumford, Picard groups of moduli problems {#ecag-0328}

[Picard groups of moduli problems](http://www.mathcs.emory.edu/~brussel/Scans/mumfordpicard.pdf) and
[Daniel Litt's expository notes](https://www.daniellitt.com/#/expository-notes/).
...
- Descent for isomorphisms (between two `things')
- Descent for auotomorphisms (defining a `thing')
...
- finite Galois covers of a curve of genus $\geq 1$.
...
- $$\mathbb{P}^{3}=\mathrm{Proj}(k[x,y,z,w])$$
- two curves in $\mathbb{P}^{3}$ 
$$C=\mathrm{Proj}(k[x,y,z,w]/(xy-z^{2},w))$$ $$D=\mathrm{Proj}(k[x,y,z,w]/(xy-w^{2},z)).$$
- they intersect at 
$$P=[1,0,0,0], Q=[0,1,0,0].$$
- two lines in $\mathbb{P}^{3}$
$$l_{1}:[x,x,z,z], l_{2}:[x,-x,z,-z].$$
- a group action of $G=\mathbb{Z}/2\mathbb{Z}=\{1,g\}$ on $\mathbb{P}^{3}$
$$g\bullet [x,y,z,w]=[y,x,w,z].$$
- $l_{1}\cup l_{2}$ is the fixed locus of the $G$-action, thus $G$ acts freely on  $$Y=\mathbb{P}^{3}\setminus\{l_{1}\cup l_{2}\}.$$
- quotient $S=Y/G$ exists as a quasi-projective scheme, explicitly $S$ is the image of the open subscheme $Y$ under the morphism 
...
- Hironaka's construction on $Y$(previously, Hironaka's construction starts with $\mathbb{P}^{3}$, here we delete $l_{1}\cup l_{2}$ to make it easier to talk about quotient, nothing essential changes), denote the complete but not projective variety over $Y$ be to 
$$\pi: V_{Y}\rightarrow Y.$$
- an open cover(in any sense you like) of $Y$.
$$X=(Y-P)\sqcup (Y-Q).$$
...
- descent for affine morphisms
- relative descent datum and group actions.
- descent datum and quotients.
...
- [Stacks project, Tag 08KE](http://stacks.math.columbia.edu/tag/08KE)
- [Stack project, Tag 08KF](http://stacks.math.columbia.edu/tag/08KF)
- [Construction of Hilbert and Quo schemes](https://perso.univ-rennes1.fr/matthieu.romagny/GT_Hilb/Nitsure_Construction_of_Hilbert_and_Quot_schemes.pdf).
- [Notes on Grothendieck topologies, fibered categories and descent theory](https://ncatlab.org/ericforgy/published/Notes+on+Grothendieck+Topologies,+Fibered+Categories+and+Descent+Theory).
- [Notes on Grothendieck topologies, fibered categories and descent theory](https://arxiv.org/pdf/math/0412512.pdf)
...
- [Kleiman's example](https://books.google.com/books?id=jAWVmIz80A4C&pg=PA11&lpg=PA11&dq=Kleiman+example+nonreduced++nonprojective&source=bl&ots=AJbeQACcC9&sig=bWwICxG1h_sMxZjwRGJ1BeKajus&hl=en&sa=X&ved=0ahUKEwjIy8vi1OvTAhVh9IMKHTGJAYYQ6AEIIjAA#v=onepage&q=Kleiman example nonreduced nonprojective&f=false).
- Mumford , the invariant ring finitely generated, appendix in GIT.

## Hodge theory

### Remark: singular (co)homology, sheaf (co)homology, de Rham cohomology an Hodge decomposition {#ecag-0349}

- We want to clarify the following concepts
- $H^{*}(X,\mathbb{Z})$ and $H^{*}(X, \underline{\mathbb{Z}})$.(Same question for $\mathbb{C}, \mathbb{R}$)
- what do we mean by $d\bar{z}$ when we're talking about an algebraic(or holomorphic) curve?
- $H^{n}(X,\mathbb{C})\cong \oplus_{p+q=n} H^{p,q}(X)$ and $H^{n}(X,\underline{\mathbb{C}})\cong \oplus_{p+q=n} H^{p}(X, \wedge^{q}\Omega_{X})$?
- Do we have $H^{p}(X, \wedge^{q}\Omega_{X})=\overline{H^{q}(X, \wedge^{p}\Omega_{X})}$, in what sense?
- Any relations between $H^{*}(X, \wedge^{k}\Omega_{X})$, $H_{dR}^{*}(X, \mathbb{C})$ and $H^{*}(X,\mathbb{C})$?
- algebraic description of the Betti lattice $H^{*}(X,\mathbb{Z})\rightarrow H^{*}(X,\mathbb{C})$?
...
This example is provided by William Lang in his thesis `Quasi-elliptic surfaces in characteristic three', I read this from Alex Youcis's blog post [here](https://ayoucis.wordpress.com/2015/07/22/algebraic-de-rham-cohomology-and-the-degeneration-of-the-hodge-spectral-sequencethe/#more-3561).
...
### Example: Family of elliptic curves, Gauss-Manin connection and Picard-Fucus function {#ecag-0355}
