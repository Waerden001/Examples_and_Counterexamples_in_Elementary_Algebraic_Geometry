...
- The global Frobenius $F_{\overline{X}}$ is $f\mapsto f^{p}$, which means maps every function to its $p$-th power. Note that this is a $k$-linear morphism, which is not true for $\mathbf{F}_{q}$.
- The relative Frobenius is given by $1_{\mathrm{Spec}(\overline{k})}\times_{k}F_{X}$. Note that this is a $\overline{k}$-linear map because $a^{p}=a$ on the right-hand side, which is not true for $\mathbf{F}_{q}$, thus the problem of $X^{(p)}$ always arises when the base field is not just merely $\mathbf{F}_{p}$.
- The arithmetic Frobenius. $F_{\overline{k}}\times_{k} 1_{X}$.
- The geometric Frobenius. $F_{\overline{k}}^{-1}\otimes 1_{X}$.
...
[Some remarks on Frobenius and Lefschetz
in ´etale cohomology](http://www.math.mcgill.ca/goren/SeminarOnCohomology/Frobenius.pdf).
...
&=\zeta(s)L(\chi, s),
\end{aligned}$$
where $\zeta(s)$ is the Riemann zeta function, $L(\chi, s)$ is the Dirichlet $L$-series for the quadratic character $\Chi: (\mathbf{Z}/5\mathbf{Z})^{\times}\rightarrow \mathbf{C}^{\times}$ given by $(\frac{}{p})$, namely $L(\chi, s):=\sum_{n=1}^{\infty}\frac{\chi(n)}{n^{s}}$.
