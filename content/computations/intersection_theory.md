...
Now use the fact that $$i_{*}[\mathcal{M}]^{\vir}=c_{\mathrm{top}}(E)=7776[\text{pt on $\mathbf{P}^{5}$}]$$
We get 
...
First note that $F_{3}$ is nothing but the projectivization of the tautological bundle $\mathcal{V}$ on $\operatorname{Gr}(2, \mathbf{C}^{3})$. We denote it by $\mathbf{P}(\mathcal{V})$. Since the Schubert cells give $\mathbf{P}(\mathcal{V})$ a CW-structure, the cohomology ring is the same as the Chow ring. Let $\zeta=c_{1}(\mathcal{O}_{\mathbf{P}(\mathcal{V})}(1))$, then $$A^{*}(\mathbf{P}(\mathcal{V}))=A^{*}(\operatorname{Gr}(1, \mathbf{C}^{3}))[\zeta]/(\zeta^{2}+c_{1}(\mathcal{V})\zeta+c_{2}(\mathcal{V})).$$
